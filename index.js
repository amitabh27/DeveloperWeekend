...
const second = (source, callback) => {
        request('https://packup-bot.herokuapp.com/n/food/'+source+'/GoogleKeyHere', function (error, response, body) {
			if (!error && response.statusCode == 200) 
			{
				console.log(body);
				var o=JSON.parse(body);
...
const handlers = {
    ...
    'myName': function(){
        //var name = this.event.request.intent.slots.vinayak.value;
	...
	'nearbyfood': function(){
		...
		var source=this.event.request.intent.slots.food.value;

		
		second(source,  (err, res) => {
